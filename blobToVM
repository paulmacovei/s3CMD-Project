...
	Output will be file system using URL from above steps, username will be “storage account name” from Access Keys tab and password will be “storage account key” from Access Key tab
•	Path = //myaccountname.file.core.windows.net/mysharename
	Once this is done it should copy the specified blob file over to the file share at regular intervals which will be accessible directly on the VM through the mountpoint
	If you wanted to regularly copy over from mountpoint to the local VM HD, you’ll need to do create a cron job to run a script regularly to copy files
•	crontab –e
...
o	Mount “File Share”
	sudo mount -t cifs //myaccountname.file.core.windows.net/mysharename /mnt/mountpoint -o vers=3.0,user=myaccountname,password=StorageAccountKeyEndingIn==,dir_mode=0777,file_mode=0777
...
o	Once all the above is complete your VM will have a direct connection to a Storage File Share (mounted like another HD) wherein you can place your SAP Dump file from blob and it will be accessible on the VM through the File Share directory @ /mnt/mountpoint
``
